...
# Rotina de deploy baseada em commit hash
# Não sobrepõe scripts existentes
...
rollback() {
    ...
    [[ -z "$target_commit" ]] && error "Especifique o commit hash para rollback"
    
    local account=$(aws sts get-caller-identity --query Account --output text)
    local ecr_uri="$account.dkr.ecr.$REGION.amazonaws.com/$ECR_REPO"
    
    # Verificar se imagem existe
    aws ecr describe-images --repository-name $ECR_REPO --region $REGION --image-ids imageTag=$target_commit >/dev/null 2>&1 || error "Imagem $target_commit não encontrada no ECR"
    
    log "Fazendo rollback para commit: $target_commit"
    ...
    local new_temp_file=$(mktemp)
    jq --arg img "$ecr_uri:$target_commit" '
        .containerDefinitions[0].image = $img |
        del(.taskDefinitionArn, .revision, .status, .requiresAttributes, .placementConstraints, .compatibilities, .registeredAt, .registeredBy)
    ' "$temp_file" > "$new_temp_file"
    ...
    success "Versão: $target_commit (revision: $revision)"
}
...
case "${1:-}" in
    ...
    *)
        ...
        echo "  rollback, r <hash>   Rollback para versão específica"
        echo
        echo "Exemplos:"
        echo "  $0 preview"
        echo "  $0 rollback 0221805"
        ;;
esac
